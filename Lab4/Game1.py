...
# Строки 350-383 - Флаг = -1. Оформление остальной части главного меню: кнопки с режимами, выход, информация
	if flag == 0:
		...
		if pre_counter < 400:
			rect(screen, (0, 0, 0), (60 + (pre_counter-220)*6.6, 230, 10000, 70))
		...
		if pre_counter <= 600:
			rect(screen, (0, 0, 0), (40 + (pre_counter-420)*4.2, 360, 4000, 70))
			rect(screen, (0, 0, 0), (560, 360, 10000, 70))
		if 600 < pre_counter < 800:
			rect(screen, (0, 0, 0), (560 + (pre_counter-620)*4.2, 360, 4000, 70))
		...
		if pre_counter < 1000:
			rect(screen, (0, 0, 0), (40 + (pre_counter-820)*6.6, 440, 10000, 70))
...
# Строки 453-528 - Флаг = 3. Сегмент с информацией, доступный в главном меню
	if flag == 4:
		...
		if xcount > 30:
			flag = F
# Строки 530-540 - Флаг = 4. Предостережительная надпись при попыткезакрыть игру нажатием на крестик
	for event in pygame.event.get(): 				# Виды событий во время игры
		if event.type == pygame.KEYDOWN:				# Событие - нажатие любой кнопки на клавиатуре 
			...
			if flag == -2:
				finished = True
			else:							# Иначе показать предостережительную надпись
				if flag != 4: F = flag
				flag = 4
				xcount = 0
